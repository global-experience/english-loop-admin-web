...
export function AdminDashboard({ tab }: { tab: Tab }) {
  ...
  const loadVideos = useCallback(async () => {
    ...
    setCategories(categoryData.items);
  }, [page, search, status, origin, categoryId, sort]);
      ...
      <main className="workspace">
        ...
        {loading ? <div className="loading-state"><LoaderCircle className="spin" /><p>데이터를 불러오는 중입니다.</p></div> : (
          <>
            ...
            {tab === "videos" && <VideosPanel videos={videos} categories={categories} origin={origin} setOrigin={(next) => { handlePageReset(); setOrigin(next); }} status={status} setStatus={(next) => { handlePageReset(); setStatus(next); }} categoryId={categoryId} setCategoryId={(next) => { handlePageReset(); setCategoryId(next); }} sort={sort} setSort={(next) => { handlePageReset(); setSort(next); }} search={search} setSearch={(next) => { handlePageReset(); setSearch(next); }} reload={loadVideos} onError={handleError} total={total} page={page} setPage={handlePageChange} onNotice={setNotice} />}
...
function VideosPanel(props: { videos: FeedVideo[]; categories: FeedCategory[]; origin: VideoOrigin; setOrigin: (value: VideoOrigin) => void; status: VideoStatus | ""; setStatus: (status: VideoStatus | "") => void; categoryId: string; setCategoryId: (value: string) => void; sort: VideoSort; setSort: (value: VideoSort) => void; search: string; setSearch: (value: string) => void; reload: () => Promise<void>; onError: (error: unknown) => void; total: number; page: number; setPage: (page: number) => void; onNotice: (message: string) => void }) {
  const { videos, categories, origin, setOrigin, status, setStatus, categoryId, setCategoryId, sort, setSort, search, setSearch, reload, onError, total, page, setPage, onNotice } = props;
  const [busy, setBusy] = useState("");
  ...
  useEffect(() => {
    setSelected([]);
  }, [page, status, origin, categoryId, sort, search]);
